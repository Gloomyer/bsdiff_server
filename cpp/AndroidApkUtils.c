...
{
  char *argv[4] = {"BsDiff"};
  argv[1] = (char *)(*env)->GetStringUTFChars(env, _old_apk_path, NULL);
  argv[2] = (char *)(*env)->GetStringUTFChars(env, _new_apk_path, NULL);
  argv[3] = (char *)(*env)->GetStringUTFChars(env, _opt_patch_path, NULL);
  int ret = diff_patch(4, argv);

  (*env)->ReleaseStringUTFChars(env, _old_apk_path, argv[1]);
  (*env)->ReleaseStringUTFChars(env, _new_apk_path, argv[2]);
  (*env)->ReleaseStringUTFChars(env, _opt_patch_path, argv[3]);
  return ret;
}
